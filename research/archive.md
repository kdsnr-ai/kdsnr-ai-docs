...
meta: Infrastructure · 2026-07-28 · 송연태 · 권준희
---
...
최근의 시험이나 직접 작업한 모의고사의 경우 어떤 시험지인지를 기억하거나, 문제지를 모아둔 파일에 문제의 키워드를 검색해서 찾는 형태입니다. 이것으로 해결되는 일도 많습니다.
...
> 이 절의 예시는 모두 **실제로 돌려 본 결과**입니다. 대상은 아카이브에 들어 있는 생명과학Ⅰ 수능 기출 10개년(2017~2026학년도, 200문항)입니다.
...
| 갈래 | 걸 수 있는 조건 |
|---|---|
| 시험 | 출처(평가원 · 교육청 · 강대 · EBS · 사설) · 시험 구분(수능 · 모의평가 · 학력평가 · 강K) · 학년도 · 회차 · 학년 |
...
옛 교육과정의 가·나형, 수학A·B, 법과정치도 그 이름 그대로 보존됩니다. "정답률 40% 이하" 같은 조건이 입력 정보에 해당합니다. 조건은 아는 만큼만 넣으면 되고, 여러 개를 넣으면 모두 만족하는 문항만 나옵니다.
...
조건을 걸어 실제로 받은 결과입니다. 파일을 열어 보지 않아도 문항이 그대로 따라옵니다.

![조건만으로 찾은 가계도 문항 — 셋 다 19번 자리에 있습니다](archive-filter.svg)
...
![문장 하나로 찾은 골격근 수축 문항 — 단원을 지정하지 않았는데 셋 다 같은 단원입니다](archive-content.svg)

단어를 맞춰 보는 것이 아닙니다. 두 번째 질문이 찾아온 문항 중 **"백신"이라는 단어가 실제로 들어 있는 것은 2019학년도 10번 하나뿐**이고, 나머지는 항체가 만들어지는 과정을 다룬다는 이유로 올라왔습니다. 반대로 조건에 맞지 않으면 200문항 중 3건만 남기고 나머지를 전부 버립니다.
...
문항을 **이미지와 글 양쪽으로** 읽어 그 특징을 하나의 숫자 묶음으로 저장해 두고, 입력한 문항과 가장 가까운 것을 찾는 방식입니다. 글로 옮기기 어려운 **그래프의 모양, 도형의 배치, 지면의 구조**가 이미지 쪽에, 개념과 용어가 글 쪽에 담겨 서로를 보완합니다.
...
아래는 실제로 돌려 본 것입니다. **아카이브에 없는 문항**을 질의로 넣었습니다 — 2025학년도 강대모의고사 1회 생명과학Ⅰ 19번을 그대로 넣고, 수능 기출 10개년에서 비슷한 문항을 찾게 했습니다.

![강대모의고사 1회 19번을 질의로 넣어 찾은 수능 기출 문항](archive-similar.svg)

네 문항이 모두 **가계도 그림과 DNA 상대량 표를 함께 제시하는 같은 구조**입니다. 발문의 첫 문장까지 거의 겹칩니다. 그리고 찾아온 셋이 전부 **수능 19번**입니다 — 이 유형이 어느 자리에 놓이는지가 검색 결과만으로 드러납니다.

세 가지는 한 번에 섞어 쓸 수 있습니다. 문항을 넣어 비슷한 것을 모은 뒤, 문장으로 그 안에서 다시 걸러내는 식입니다. 유사도 순서는 그대로 두고 조건에 맞지 않는 것만 빠집니다 — 물어본 것의 중심이 "비슷한 문항"이기 때문입니다.

![문항과 문장을 함께 넣은 결과 — 비슷한 문항 중에서 조건에 맞는 것만 남습니다](archive-combo.svg)
...
- 국어처럼 지문 하나에 문항이 여럿 달린 경우, **세트 안에 조건에 맞는 문항이 하나라도 있으면** 그 세트가 나옵니다.
...
Archive에는 [Pipeline](pipeline.html)이 추출한 문항이 그대로 들어갑니다. 발문·자료·보기·선지가 구조를 유지한 채 저장되므로, 꺼내서 바로 HWPX로 조판하거나 이미지로 변환할 수 있습니다. 검색 결과로 받은 문항은 **다시 편집 가능한 형태**라는 뜻입니다.
...
같은 시험지라도 원고와 인쇄본은 조판·오탈자가 다르므로 자료 형태를 따로 표시해 둡니다.

정답률처럼 **문항을 아무리 정확히 읽어도 알 수 없는 정보**가 있고, 이것이 붙어 있는지에 따라 검색으로 답할 수 있는 질문의 범위가 달라집니다. 이 정보는 분석지 양식으로 한꺼번에 반영할 수 있습니다.
...
검색이 다루는 항목이 문항보다 적은 것은 국어 지문 세트가 한 항목에 여러 문항을 품기 때문입니다. 아래는 모두 항목 기준입니다.

| 출처 | 항목 | 과목 | 항목 |
|---|---|---|---|
| 평가원 | 12,807 | 과학 | 10,271 |
| 교육청 | 8,959 | 수학 | 6,069 |
| 강대 | 1,734 | 사회 | 5,553 |
| 사설(더프) | 120 | 국어 | 1,727 |
...
| 붙어 있는 정보 | 범위 |
|---|---|
| 이미지와 본문을 합친 벡터 | 23,620개 **전부** (대기 0건) |
| 정답률 | 20,177개 (85%) |
| 선택률 | 19,178개 (81%) |
| 단원 · 키워드 | 생명과학Ⅰ 수능 10개년 200문항 |

**세 가지 검색 모두 적재된 전 과목에서 동작합니다.** 단원·키워드는 넓히는 중이고, 수능특강 등 EBS 연계 교재는 준비 중입니다. 고1·고2 자료는 통합과학 194문항이 먼저 들어와 있습니다.

## 5. 자료를 안전하게 나누기

모든 문항을 모두에게 열 수는 없습니다. 기출은 비교적 자유롭게 쓸 수 있지만 강K 문항이나 사설 자료는 그렇지 않습니다.

Archive는 **발급하는 열쇠마다 접근 범위를 다르게** 줄 수 있습니다. 출처·과목·시험·학년도·회차·자료 형태를 기준으로 범위를 정하면, 그 범위 밖의 문항은 검색에도 나오지 않고 직접 번호를 알아도 열리지 않습니다.
